"""Groq transport adapter for Strands OpenAIModel in Benchbook.

Derived with attribution from Borrowed Steps
(01_BORROWED_STEPS/services/agent/src/borrowed_steps/infrastructure/groq_model.py)
...
1. Strict constructor arguments: explicit API key required, no environment lookups,
   pinned endpoint (https://api.groq.com/openai/v1) and model (openai/gpt-oss-20b).
...
3. Monotonic operation deadline (capped at 110.0s) and per-request timeout (capped at 60.0s).
